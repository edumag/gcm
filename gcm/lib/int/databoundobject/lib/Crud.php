...
class Crud extends DataBoundObject {
   ...
   *
   * Para ver las posibilidades paginarPDO
   */
  ...
  protected $evento_guardar = FALSE;     ///< Método a lanzar al guardar registro, recibe identificador de registro como parametro
  protected $evento_modificar = FALSE;   ///< Método a lanzar al modificar registro, recibe identificador de registro como parametro
  protected $evento_borrar  = FALSE;     ///< Método a lanzar al borrar registro, recibe identificador de registro como parametro
  ...
  /**
   * Directorio de imágenes utilizadas por Array2Table
   */
  ...
  /**
   * Podemos definir un método personalizado para la visualización de los registros 
   ...
   * @param $tipo_tabla Tipo de tabla, @see $tipo_tabla.
   */
   ...
   *
   * @todo Los casos que están comentados hay que buscar la manera de implementarlos.
   *
   */
        ...
        if ( $this->save() ) {
          ...
          if ( $this->combinar_tablas ) {
            ...
            foreach ( $this->combinar_tablas as $combinatoria ) {
              ...
              // Recogemos identificadores de registros para posteriormente 
              // añadirlos a la tabla que los relaciona.
              ...
              // Recogemos nuevos registros relacionados insertados en el formulario y
              ...
              for ( $index = 0 ; $index < $numero_registros_formulario ; $index++ ) {
                ...
                if ( isset($resultado[$nombre_campo_identificativo][$index]) ) {
                  $identificadores_contenido_insertados[] = $resultado[$nombre_campo_identificativo][$index];
                } else {
                  break ;
                }
...
<?php
        ...
        if ( $this->ID ) {
          $condicion_combinatoria = "$campo_relacion = $this->ID";
          $relacionados = $rel->find($condicion_combinatoria, array($campo_combinatoria));
          // generamos condición para recoger los relacionados.
          foreach( $relacionados as $relacion ) {
            foreach( $relacion as $rel ) {
              $ids_relacionados .= $rel.', ';
            }
          }
          $ids_relacionados = rtrim($ids_relacionados,', ');
          ...
          $condicion_lista = FALSE;
            ...
            ?>
            <fieldset>
               <legend  accesskey="c"><?php echo literal('Seleccionar para eliminar de la lista') ?></legend>
               <div  class="posibles_registros registros_relacionados">
                  <ul id="relacionados_<?php echo $tabla_contenido;?>">

                     <?php if ( $ids_relacionados ) { ?>
                     <?php $condicion_lista = "id IN ($ids_relacionados)"; ?>
                     <?php $relacionados = $rel_contenido->find($condicion_lista); ?>
                     <?php if ( $relacionados ) { ?>
                     <?php foreach ( $relacionados as $relacionado ) { ?>
                     <?php $id_relacionado = $relacionado[$campo_contenido]; ?>
                     <li id="li_relacionado_<?php echo $tabla_contenido;?>-<?php echo $id_relacionado ?>">
                        <?php $conta=0; $salida = ''; foreach($relacionado as $campo => $valor ) { $conta++ ; ?>
                        <?php if ( $campo != $campo_contenido ) $salida .= literal($valor); ?>
                        <?php if ( $conta > 1 ) { break; } // Solo añadimos el siguiente campo al identificador ?> 
                        <?php } ?>
                        <a href="javascript:<?php echo $tabla_contenido; ?>.eliminar_registro(<?php echo $id_relacionado; ?>,'<?php echo str_replace("'","\'",$salida) ?>')">
                           <?php echo $salida ?>
                        </a>
                        <input type="hidden" name="<?php echo $tabla_contenido.'_'.$tabla_contenido.'_id[]' ?>" value="<?php echo $id_relacionado ?>" />
                     </li>
                     <?php } ?>
                     <?php } ?>
                  </ul>
        ...
        $condicion_lista = ( $condicion_lista ) ? "NOT ".$condicion_lista : FALSE ;

        $posibles = $rel_contenido->find($condicion_lista, FALSE, $this->defaultorder);
?>
            <fieldset>
               <legend  accesskey="c"><?php echo literal('Seleccionar existente') ?></legend>
               <div class="posibles_registros">
                  <ul id="posibles_registros_<?php echo $tabla_contenido;?>">
                     <?php if ( $posibles ) { ?>
